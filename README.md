...
shasum -a 256 ctk_v0.7.0_darwin_arm64.tar.gz
tar -xzf ctk_v0.7.0_darwin_arm64.tar.gz
mkdir -p "$HOME/.local/bin"
install -m 0755 ctk "$HOME/.local/bin/ctk"
```

Compare the displayed SHA-256 value with the corresponding line in
`checksums.txt`. Use `ctk_v0.7.0_darwin_amd64.tar.gz` on an Intel Mac and add
...
Get-FileHash .\ctk_v0.7.0_windows_amd64.zip -Algorithm SHA256
Expand-Archive .\ctk_v0.7.0_windows_amd64.zip -DestinationPath "$env:LOCALAPPDATA\CTK\bin"
```
...
these examples identify the current v0.7.0 Release; keep the archive,
